...
class TakeANapTool(BaseTool):
    name: str = "take_a_nap"
    description: str = (
        "A dummy function that takes a nap for 30 seconds and then wakes up with a friendly message. Use this to test long-running function calls and non-blocking execution."
    )
    ...
    async def execute(self, session, queue) -> dict[str, Any]:
        return {
            "status": "SUCCESS",
    ...
    def implementation(self, session, queue):
        return super().implementation(session, queue)
